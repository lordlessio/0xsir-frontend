<template>
  <section class="text-center relative sir-overview" :class="{ 'info': !unInfo, 'download': download }">
    ...
    <div v-show="download" class="text-left overview-download-header">
      ...
      <p v-if="overviewDatas.info.name" class="overview-download-name">[{{ overviewDatas.info.name }}]</p>
      <p class="TTFontBold text-break overview-download-address">{{ address }}</p>
      <ul v-if="overviewDatas.info.cat.length" class="d-flex flex-row overview-contract-cats download-contract-cats">
        <li v-for="(cat, index) of overviewDatas.info.cat" :key="index">
          {{ cat }}
        </li>
      </ul>
...
export default {
  ...
  methods: {
    ...
    drawCloud (words = this.words) {
      ...
      console.log('---', words.map(item => item.name))
      ...
      const layout = Cloud()
        ...
        .words(
          words.map(function (d) {
            ...
            return { text: d.name, size: _size, count: d.count }
          })
        )
        // .random(function () {
        //   return 0
        ...
        .rotate(function () {
          ...
          return [0, 90][Math.round(Math.random())]
        })
        .font('Impact')
        .fontSize(function (d) {
          return d.size
